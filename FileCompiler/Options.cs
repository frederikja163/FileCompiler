...
internal sealed class Options
{
    ...
    [Option('o', "output", Default = "./Output", HelpText = "The output directory of the files.")]
    ...
    [Option("line", Default = "\\$<(?<command>\\S+)(?: (?:(?:\"\"(?<param>.*)\"\")|(?<param>\\S+)))*>", HelpText = "The regex for inserting a file on a single line.")]
    public string LineRegex { get; set; } = "";
    
    [Option("file", Default = "\\${(?<command>\\S+)(?: (?:(?:\"\"(?<param>.*)\"\")|(?<param>\\S+)))*}", HelpText = "The regex for inserting a file.")]
